...
sealed interface AddToCollectionEvent {
    data object EntryAdded : AddToCollectionEvent
}
...
class AddToCollectionViewModel(
    ...
    fun updateQuery(value: String) {
        ...
        _uiState.update {
            it.copy(
                ...
                printings = if (value.isBlank()) emptyList() else it.printings,
                errorMessage = null,
            )
        }
    }
    ...
    fun updateDisplayLanguage(value: CardLanguage) {
        ...
        _uiState.update {
            it.copy(
                ...
                isLoading = it.query.isNotBlank(),
                printings = if (it.query.isBlank()) emptyList() else it.printings,
                errorMessage = null,
            )
        }
    }
    ...
    /** Schedules constrained background work. The UI continues to read catalog data from Room. */
    fun requestCatalogUpdate() {
        ...
        viewModelScope.launch {
            ...
            try {
                catalogUpdateScheduler.enqueue()
            } catch (error: Throwable) {
                if (error is CancellationException) throw error
                _uiState.update {
                    it.copy(
                        errorMessage = error.message ?: "The catalog update could not be scheduled.",
                    )
                }
            } finally {
                _uiState.update { it.copy(isRequestingCatalogUpdate = false) }
            }
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.update {
                    it.copy(
                        isSaving = false,
                        errorMessage = error.message ?: "The card could not be added to your collection.",
                    )
                }
            }
        }
    ...
    private fun load() {
        ...
        catalogJob = viewModelScope.launch {
            try {
                combine(query, displayLanguage) { currentQuery, language ->
                    currentQuery.trim() to language
                }
                    .flatMapLatest { (currentQuery, language) ->
                        if (currentQuery.isBlank()) {
                            flowOf(emptyList<CatalogPrintingSummary>())
                        } else {
                            catalogRepository.observePrintings(currentQuery, language)
                        }
                    }
                    .collect { printings ->
                        _uiState.update {
        ...
        catalogStatusJob = viewModelScope.launch {
            try {
                catalogRepository.observeCatalogUpdateStatus().collect { status ->
                    _uiState.update { it.copy(catalogUpdateStatus = status) }
                }
            } catch (error: Throwable) {
                if (error is CancellationException) throw error
                _uiState.update {
                    it.copy(
                        errorMessage = error.message ?: "The catalog update status could not be loaded.",
                    )
                }
            }
        }
